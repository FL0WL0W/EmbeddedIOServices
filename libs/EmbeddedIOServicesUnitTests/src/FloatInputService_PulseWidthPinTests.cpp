...
namespace UnitTests
{
	class FloatInputService_PulseWidthPinTest : public ::testing::Test 
	{
		protected:
		...
		FloatInputService_PulseWidthPinTest() 
		{
		...
		~FloatInputService_PulseWidthPinTest() override 
		{
			free(_floatInputService);
		}
	};

	TEST_F(FloatInputService_PulseWidthPinTest, WhenGettingValue_ThenCorrectValueIsReturned)
	{
		...
		ASSERT_NEAR(0.05f, _floatInputService->Value, 0.001f);

		pwmValue = { 0.025f, 0.004f };
		EXPECT_CALL(_pwmService, ReadPin(1)).Times(1).WillOnce(Return(pwmValue));
		_floatInputService->ReadValue();
		ASSERT_NEAR(0.004f, _floatInputService->Value, 0.001f);

		pwmValue = { 0.05f, 0.04f };
		EXPECT_CALL(_pwmService, ReadPin(1)).Times(1).WillOnce(Return(pwmValue));
		_floatInputService->ReadValue();
		ASSERT_NEAR(0.04f, _floatInputService->Value, 0.001f);
