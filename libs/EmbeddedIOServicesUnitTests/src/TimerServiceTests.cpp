...
namespace UnitTests
{
	...
	TEST(TimerService, WhenAddingLaterTask_ThenFirstTaskIsCalledFirst)
	{
		...
		ASSERT_EQ((void *)task1, (void *)timerService.ScheduledTask) << "ScheduledTask not set to first after new later task added";
		ASSERT_EQ((void *)task2, (void *)timerService.ScheduledTask->NextTask) << "NextTask not set to second  after new later task added";
		...
		ASSERT_EQ((void *)task2, (void *)timerService.ScheduledTask) << "Schedule not set to second after first task called";
		...
		timerService.ReturnCallBack();
		ASSERT_EQ(1, lastCallBack) << "first callback not called";
		ASSERT_EQ((void *)task2, (void *)timerService.ScheduledTask) << "Schedule tick not set to second overflow task after first task called";

		EXPECT_CALL(timerService, GetTick())
			.WillRepeatedly(Return(300));
		timerService.ReturnCallBack();
		ASSERT_EQ(2, lastCallBack) << "second callback not called";
		ASSERT_EQ(0, timerService.ScheduledTask) << "Schedule tick not set to second overflow task after second task called";
