...
namespace Variables
{
	...
	public:		
		...
		static IVariable *Create(Service::ServiceLocator * const &serviceLocator, const void *config, unsigned int &sizeOut)
		{
			...
			RET *variable = serviceLocator->LocateAndCast<RET>(BUILDER_VARIABLES, variableId);
			if(variable == 0)
			{
				variable = (RET *)malloc(sizeof(RET));
				serviceLocator->Register(BUILDER_VARIABLES, variableId, variable);
			}
			...
			return new Variable_Operation<RET, PARAMS...>(operation, variable, serviceLocator->LocateAndCast<PARAMS>(BUILDER_VARIABLES, Service::IService::CastAndOffset<uint16_t>(config, sizeOut))...);
		}
	...
	public:		
		...
		static IVariable *Create(Service::ServiceLocator * const &serviceLocator, const void *config, unsigned int &sizeOut)
		{
			...
			return new Variable_Operation<void, PARAMS...>(operation, serviceLocator->LocateAndCast<PARAMS>(BUILDER_VARIABLES, Service::IService::CastAndOffset<uint16_t>(config, sizeOut))...);
		}
		ISERVICE_REGISTERFACTORY_H_TEMPLATE
	};
}
