...
{	
	void CommunicationHandler_Prefix::RegisterHandler(ICommunicationHandler *handler, const void *prefix, const size_t prefixLength, const bool handlesData)
	{
		if(_receiveCallBackMap.find(handler) != _receiveCallBackMap.end())
			return;

		_receiveCallBackMap.insert(std::pair<void *, communication_prefix_receive_callback_t>(
			handler, 
	...
	void CommunicationHandler_Prefix::UnRegisterHandler(ICommunicationHandler *handler)
	{
		communication_prefix_receive_callback_map_t::iterator it = _receiveCallBackMap.find(handler);
