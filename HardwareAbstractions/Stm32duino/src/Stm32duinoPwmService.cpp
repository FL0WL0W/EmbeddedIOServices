...
#include "Stm32duinoPwmService.h"
#include <algorithm>
#include <math.h>
...
namespace Stm32
{
	...
	TimAndChannel Stm32duinoPwmPinToTimAndChannel(uint16_t pin)
	{
		switch (pin)
		{
#ifdef PWM_TIM1_CC1
	...
	void Stm32duinoPwmService::InitPin(uint16_t pin, PinDirection direction, uint16_t minFrequency)
	{
		if (pin == 0)
			return;
					
		TimAndChannel timAndChannel = Stm32duinoPwmPinToTimAndChannel(pin);
		...
		TimInit(timAndChannel.Index, prescaler, static_cast<uint16_t>(ceil(minFrequency / (prescaler+1))));
	...
	PwmValue Stm32duinoPwmService::ReadPin(uint16_t pin)
	{
	...
	void Stm32duinoPwmService::WritePin(uint16_t pin, PwmValue value)
	{
		if (pin == 0)
			return;
