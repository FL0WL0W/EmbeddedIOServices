...
namespace UnitTests
{
    ...
    protected:
        ...
        const CANIdentifier_t _listenIds[1] = { { 0x700, 0 } };
        const CANIdentifier_t _transmitIds[1] = { { 0x708, 0 } };
    };

    TEST_F(CommunicationService_ISOTPTests, SendsSingleFrameForSevenBytesOrLess)
    {
        CommunicationService_ISOTP service(&_canService, _listenIds, 1, _transmitIds, 1);
        const uint8_t payload[3] = { 0x22, 0xF1, 0x90 };

        EXPECT_CALL(_canService, Send(_transmitIds[0], _, 4))
    ...
    TEST_F(CommunicationService_ISOTPTests, SendsFirstFrameAndConsecutiveFramesForLongPayload)
    {
        ...
        InSequence sequence;
        EXPECT_CALL(_canService, Send(_transmitIds[0], _, 8))
            .WillOnce([](const CANIdentifier_t identifier, const CANData_t data, const uint8_t dataLength) {
        ...
        EXPECT_CALL(_canService, Send(_transmitIds[0], _, 5))
            .WillOnce([](const CANIdentifier_t identifier, const CANData_t data, const uint8_t dataLength) {
    ...
    TEST_F(CommunicationService_ISOTPTests, ReceivesSingleFramePayload)
    {
        ...
        _canService.Receive(_listenIds[0], frame, 4);
    ...
    TEST_F(CommunicationService_ISOTPTests, ReassemblesMultiFramePayloadAndSendsFlowControl)
    {
        ...
        EXPECT_CALL(_canService, Send(_transmitIds[0], _, 3))
            .WillOnce([](const CANIdentifier_t identifier, const CANData_t data, const uint8_t dataLength) {
        ...
        _canService.Receive(_listenIds[0], firstFrame, 8);
        _canService.Receive(_listenIds[0], consecutiveFrame, 5);
